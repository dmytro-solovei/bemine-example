  ...
  if ($confirm) {
    $.ajax({
      ...
      success: function success(response) {
        var elem = $(_this).parent().closest('.existing-gallery-block');
...
$(document).on('click', '#add_gallery', function () {
  ...
  var allElements = $(_this).parent().closest('.product-gallery').find('#gallery_block .gallery-block-row');
  var lastBlockCount = Number(allElements.last().attr('data-gallery-block-count'));
...
$(document).on('click', '.open-edit-form', function (e) {
  ...
  var $createBlockForm = $(".create-block[data-id=".concat(id, "] .create-block-content form"));
  var $createBlock = $(".create-block[data-id=".concat(id, "] .create-block-content .edit-form"));
  ...
  $.ajax({
    url: "/dsfgsfghzxdfgzdfhzdgv/section/add/edit/block/".concat(id),
    type: 'POST',
    data: {},
    success: function success(response) {
...
$(document).on('click', '.update-item', function (e) {
  var $contentEditable = $(e.currentTarget).closest('[contenteditable]');
  var id = $contentEditable.attr('data-id');
  var transformedObject = $contentEditable.find('input').toArray().reduce(function (acc, input) {
    acc[input.name] = input.value;
    return acc;
  }, {});
  $.ajax({
    type: "PUT",
    url: "/dsfgsfghzxdfgzdfhzdgv/section/".concat(id),
...
function getLanguageInputs() {
  $.ajax({
    ...
    method: "POST",
    success: function success(response) {
      var appendElements = document.querySelectorAll('.append-input');
      appendElements.forEach(function (element) {
